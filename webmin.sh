...
echo -e "Adicionando o Repositório e Assinatura, aguarde..."
	...
	apt-get install -y perl libnet-ssleay-perl openssl libauthen-pam-perl libpam-runtime &>> $LOG
	apt-get install -y libio-pty-perl apt-show-versions python &>> $LOG
	apt-get install -y gnupg gnupg2 &>>$LOG
	apt-key add jcameron-key.asc  &>> $LOG
	apt-get install -y apt-transport-https &>> $LOG
